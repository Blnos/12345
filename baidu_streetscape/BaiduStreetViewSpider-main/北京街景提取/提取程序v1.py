...
if __name__ == "__main__":
    ...
    error_fn = r'error.csv'  # 错误记录CSV文件名
    ...
    for i in range(len(data)):
        ...
        try:
            longitude = data[i][3]  # Longitude（经度）
            latitude = data[i][4]   # Latitude（纬度）
        except IndexError:
        ...
        for heading in headings:
            img_name = f"{longitude}_{latitude}_{heading}_{pitchs}.png"
        ...
        # 下载四个方向的街景图片
        for heading in headings:
            save_path = os.path.join(root, img_dir, f"{longitude}_{latitude}_{heading}_{pitchs}.png")
            # 构建街景图片URL
