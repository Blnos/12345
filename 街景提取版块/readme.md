...
## 内容
### 街景提取的主程序('北京街景提取'文件夹)是借鉴已有项目，在此照搬原项目说明：

该文件夹主要包含一个用于获取街景的脚本文件和一个输出目录。

+ 街景提取程序v1.py：该脚本为获取街景的主要程序，输入为存有wgs84坐标系下经纬度信息的csv文件。

+ dir_test文件夹：存有脚本的输入文件以及输出的街景图像和爬取失败的信息。该文件夹主要包括以下内容：
  1. point.csv是道路点经纬度信息。
  2. error文件为对应的爬取失败的点的信息。
  3. images文件夹下存放爬取成功的街景图像。

### 街景提取的辅助程序 (路网点提取文件夹)

“路网点提取”文件夹主要包含一个将路网shp格式线数据直接转化为点数据的程序“路网提取-main.py”
转换成功的点数据会输出六种相应的地图数据格式。本次处理仅需采用csv格式数据。
...
1. 利用arcgis或QGIS提取需要街景数据的经纬度信息，以csv文件存储。-（通过 路网提取-main.py 实现）
