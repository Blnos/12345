...
* **原始数据**：持有的路网数据基于 **WGS84 坐标系**（即通用 GPS 坐标）。
...
为了消除数学计算的误差，放弃了本地估算，转而使用百度官方的 Web 服务 API。
...
* **批量策略**：利用 API 的并发特性，将原始坐标点每 **100个** 打包成一组发送。
  ...
  * *效益*：百度 API 计费是按“次”计算。通过批量打包，将 **100个点的转换成本降低为 1次配额**，极大降低了大规模抓取的费用（节省 99% 成本）。
...
`
   ...
   * *优势*：该接口无需鉴权（AK），且因为传入的是官方校准过的 MC 坐标，命中率接近 100%。
